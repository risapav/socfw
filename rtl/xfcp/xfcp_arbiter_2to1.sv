 ...
 *   Pre WRITE/STREAM_WRITE/MEM_WRITE: p0_remain = COUNT bajtov payloadu.
...
module xfcp_arbiter_2to1 #(
  ...
  // Is current opcode a WRITE, STREAM_WRITE or MEM_WRITE (request has a byte payload)?
  wire p0_is_write_w = (p0_op_q == 8'(XFCP_OP_WRITE))        ||
                       (p0_op_q == 8'(XFCP_OP_STREAM_WRITE))  ||
                       (p0_op_q == 8'(XFCP_OP_MEM_WRITE));
