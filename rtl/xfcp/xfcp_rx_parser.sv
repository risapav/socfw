 ...
 * @brief Streaming parser pre XFCP request packet (v1.3+MEM).
 *
 * Formát paketu:
 *   Bajt  | Pole
 *   ------|---------------------
...
module xfcp_rx_parser #(
  ...
  (* fsm_encoding = "one-hot" *) parser_state_e state_q, state_n;
  ...
  always_comb begin
    ...
    if (state_q == S_HDR && axis_fire && s_axis_tlast && !last_hdr_byte)
      go_drop = 1'b1;
    // TLAST na poslednom header bajte s WRITE/STREAM_WRITE/MEM_WRITE: chyba payload.
    if (state_q == S_DECODE && tlast_on_last_hdr_q &&
        (dec_opcode == 8'(XFCP_OP_WRITE) ||
         dec_opcode == 8'(XFCP_OP_STREAM_WRITE) ||
         dec_opcode == 8'(XFCP_OP_MEM_WRITE)))
      go_drop = 1'b1;
    // WRITE/STREAM_WRITE/MEM_WRITE s COUNT=0 sposobuje deadlock — zahodime paket.
    if (state_q == S_DECODE &&
        (dec_opcode == 8'(XFCP_OP_WRITE) ||
         dec_opcode == 8'(XFCP_OP_STREAM_WRITE) ||
         dec_opcode == 8'(XFCP_OP_MEM_WRITE)) &&
      ...
      case (state_q)
        ...
        S_DECODE: begin
          if (dec_valid_r) begin
            ...
            // Opcodes s payload (WRITE, STREAM_WRITE, MEM_WRITE): header-first, potom S_PAYLOAD
