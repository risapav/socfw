...
module eth_rx_mac #(
  ...
  // Early header metadata — combinatorial, valid for 1 cycle when header is fully parsed.
  // Fires at the first cycle of ST_PAYLOAD (before any payload byte is emitted).
  // 5 clock cycles of margin before first m_axis_tdata beat (FCS window filling).
  output      logic        m_hdr_valid_o,    // 1-cycle pulse: header parsed
  output      logic [47:0] m_hdr_dst_mac_o,
  output      logic [47:0] m_hdr_src_mac_o,
  output      logic [15:0] m_hdr_eth_type_o,
  output      logic        m_hdr_mac_ok_o,   // DST MAC accepted (unicast/bcast match)
  ...
  // Early header: combinatorial pulse at the first cycle of ST_PAYLOAD.
  // win_cnt_q == 0 is true exactly once per frame (before the window starts filling).
  assign m_hdr_valid_o    = (state_q == ST_PAYLOAD) && (win_cnt_q == 3'd0);
  assign m_hdr_dst_mac_o  = dst_mac_q;
  assign m_hdr_src_mac_o  = src_mac_q;
  assign m_hdr_eth_type_o = eth_type_q;
  assign m_hdr_mac_ok_o   = mac_match_q;
