...
class FilesTclEmitter:
    def emit(self, *, out_dir: str, system, planned_bridges: list, project_dir: str | None = None) -> str:
        ...
        for fp in self._collect_rtl_files(system, planned_bridges):
            lines.append(f"set_global_assignment -name VERILOG_FILE {_norm(fp)}")
        ...
        if vendor.qip_files:
            ...
            for qip in sorted(vendor.qip_files):
                lines.append(f"set_global_assignment -name QIP_FILE {_norm(qip)}")

        if vendor.sdc_files:
            lines.append("")
            lines.append("# Vendor SDC files")
            for sdc in sorted(vendor.sdc_files):
                lines.append(f"set_global_assignment -name SDC_FILE {_norm(sdc)}")
