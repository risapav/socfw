...
module eth_udp_echo_test #(
  ...
  // Echo buffer — no ramstyle attribute: 512x8 inferred as M9K block RAM
  ...
  logic [ABITS-1:0] rd_ptr_r;   // read pointer (address presented to M9K)
  logic [7:0]       tx_data_q;  // M9K registered output: payload_mem_r[rd_ptr_r], 1-cycle lag
      ...
      case (fsm_r)
        ...
        ST_SEND_META: begin
          if (udp_tx_meta_ready_i) begin
            rd_ptr_r <= '0;
            // wr_ptr_r is stable; skip echo for 0-byte payload
            fsm_r    <= (wr_ptr_r == '0) ? ST_DONE : ST_SEND_LOAD;
          end
        end

        // -- One-cycle wait: M9K registered read of payload_mem_r[rd_ptr_r] --
        ST_SEND_LOAD: begin
          fsm_r <= ST_SEND_PAY;
        end
        ...
        ST_SEND_PAY: begin
          if (udp_tx_ready_i) begin
            ...
            if (rd_ptr_r == wr_ptr_r - 1'b1) begin
              ...
              fsm_r    <= ST_SEND_LOAD;  // wait one cycle for M9K output
            end
          end
        end
  ...
  // ---------------------------------------------------------------------------
  // M9K registered read — separate always_ff enables block-RAM inference.
  // tx_data_q holds payload_mem_r[rd_ptr_r] with 1-cycle lag (sampled at ST_SEND_LOAD,
  // stable and valid throughout ST_SEND_PAY).
  // ---------------------------------------------------------------------------
  always_ff @(posedge clk_i or negedge rst_ni) begin
  ...
  assign udp_tx_data_o  = tx_data_q;
