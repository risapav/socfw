#!/usr/bin/env bash
# hw_regression.sh — xfcp_test_08_caps HW regresia: UART + UDP
#
# Pouzitie:
...
echo ""
echo "=========================================="
echo "  xfcp_test_08_caps HW regresia"
...
run_test "UART transport ($UART_PORT @ $BAUD)" \
    --uart "$UART_PORT" --baud "$BAUD" --caps --rw --stream --diag

run_test "UDP  transport ($FPGA_IP:$UDP_PORT)" \
    --udp  "$FPGA_IP:$UDP_PORT" --caps --rw --stream --diag
