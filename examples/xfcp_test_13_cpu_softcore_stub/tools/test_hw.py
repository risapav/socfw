#!/usr/bin/env python3
"""
test_hw.py -- skriptovany HW test pre xfcp_test_13_cpu_softcore_stub.
...
def run_stub_test(bus, repeat=10):
    """Test CPU softcore stub: STREAM_WRITE sid=1 -> STREAM_READ sid=1."""
    PING = b"PING"
    PONG = b"PONG"
    UNKNOWN = b"ABCD"
    ERR  = b"ERR\n"

    print(f"\n{Fore.CYAN}--- CPU softcore stub test (PING->PONG, ERR, {repeat}x PING) ---")
    passed = 0
    failed = 0

    # Flush mailbox FIFO pred zaciatkom (CTRL rx_flush + tx_flush)
    CPUM_BASE = 0xFF070000
    bus.write32(CPUM_BASE + 0x04, 0x3)
    time.sleep(0.05)
    ...
    if rx == PONG:
        ...
        got = rx.hex() if rx else "TIMEOUT"
        print(f"  PING->PONG (1x):   {FAIL}  (got {got})")
        failed += 1
    ...
    for i in range(repeat):
        ...
        if rx == PONG:
            ...
            got = rx.hex() if rx else "TIMEOUT"
            print(f"  PING[{i}] fail: {got}")
            failed += 1
    passed += ok_count
    failed += (repeat - ok_count)
    status = PASS if ok_count == repeat else FAIL
    print(f"  PING->PONG ({repeat}x):  {status}  ({ok_count}/{repeat})")
...
def main():
    parser = argparse.ArgumentParser(description="xfcp_test_13_cpu_softcore_stub HW test")
    ...
    parser.add_argument("--stub",    action="store_true",
                        help="Spusti CPU stub test (PING->PONG, ERR, 10x PING)")
    ...
    print(f"\n{Fore.LIGHTWHITE_EX}=== xfcp_test_13_cpu_softcore_stub HW test  [{label}] ===")
    ...
    try:
        with bus:
            ...
            if args.stub:
                p, f = run_stub_test(bus, repeat=args.repeat)
                total_pass += p
                total_fail += f
