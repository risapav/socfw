#!/usr/bin/env bash
# hw_regression.sh -- xfcp_test_13_cpu_softcore_stub HW regresia: UART + UDP
#
# Pouzitie:
...
echo ""
echo "=========================================="
echo "  xfcp_test_13_cpu_softcore_stub HW regresia"
...
run_test "UART transport ($UART_PORT @ $BAUD)" \
    --uart "$UART_PORT" --baud "$BAUD" --caps --targets --rw --stream --cpum --stub --mem --diag

run_test "UDP  transport ($FPGA_IP:$UDP_PORT)" \
    --udp  "$FPGA_IP:$UDP_PORT" --caps --targets --rw --stream --cpum --stub --mem --diag
