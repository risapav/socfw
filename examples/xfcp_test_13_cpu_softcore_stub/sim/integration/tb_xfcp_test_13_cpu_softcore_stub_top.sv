...
// tb_xfcp_test_13_cpu_softcore_stub_top -- integration test for xfcp_test_13_cpu_softcore_stub_top.
//
...
// T18:     STREAM_WRITE sid=1 4B -> stub consume -> RX_LEVEL==0 + STREAM_READ ERR\n (stub active).
...
// T44:     STREAM_WRITE sid=1 256B -> stub consume -> RX_LEVEL==0 + TX ERR\n drain.
// T45:     TX_PUSH_DATA 4B -> STATUS[2]=tx_not_empty + TX_LEVEL==4 -> STREAM_READ -> STATUS[2]=0.
// T46:     STREAM_WRITE sid=1 8B -> stub consume -> RX_LEVEL==0 + TX ERR\n drain.
// T47:     RX underflow: RX_POP_DATA when empty -> bit[10]=1.
// T48:     STATUS sanity: STREAM_WRITE 4B -> STATUS[0]=rx_not_empty==0 (stub consumed) + TX flush.
// T49:     TX FIFO flush: TX_PUSH_DATA 4B -> TX_LEVEL==4 -> CTRL[1]=tx_flush -> TX_LEVEL==0.
// T50:     STREAM_WRITE sid=1 "PING" (4B) -> stub FSM -> STREAM_READ sid=1 "PONG".
// T51:     STREAM_WRITE sid=1 "ABCD" (4B) -> stub FSM -> STREAM_READ sid=1 "ERR\n".
// T52:     PING->PONG x4 back-to-back repeated transactions.
// T53:     STR0 sid=0 loopback isolation: unaffected by stub active on sid=1.
// T54:     STREAM_WRITE sid=1 8B (PING+extra) -> stub -> ERR\n (cmd_len != 4).

module tb_xfcp_test_13_cpu_softcore_stub_top;
  import tb_eth_pkg::*;
  ...
  // DUT instantiation
  // =========================================================================
  xfcp_test_13_cpu_softcore_stub_top #(
    .CLOCK_HZ       (CLOCK_HZ),
  ...
  initial begin
    ...
    // T18: STREAM_WRITE sid=1 4B -> stub consumes -> RX_LEVEL==0 + TX ERR\n -> drain
    $display("--- T18: STREAM_WRITE sid=1 4B -> stub consume -> RX_LEVEL==0 + drain ERR\\n ---");
    begin
      automatic logic [7:0] wr[256] = '{default: 8'h00};
      automatic logic [7:0] rd[256];
      ...
      // Stub consumed all bytes: RX_LEVEL==0
      xfcp_read(32'hFF07_0010, 8'h81);
      recv_read(rdata_t18);
      chk32(rdata_t18, 32'h0, "T18.RX_LEVEL==0_stub_consumed");
      // Stub responded ERR\n: TX_LEVEL==4
      xfcp_read(32'hFF07_0014, 8'h82);
      recv_read(rdata_t18);
      chk32(rdata_t18, 32'h4, "T18.TX_LEVEL==4_stub_ERR");
      // Drain stub response to leave TX FIFO clean
      xfcp_stream_read(8'h83, 8'h01, 16'd4);
      recv_stream_read(16'd4, rd, status);
      chk8(status,  8'h00, "T18.stream_read_status==OK");
      chk8(rd[0], 8'h45, "T18.rd[0]=='E'");
      chk8(rd[1], 8'h52, "T18.rd[1]=='R'");
      chk8(rd[2], 8'h52, "T18.rd[2]=='R'");
      chk8(rd[3], 8'h0A, "T18.rd[3]==LF");
    end
    ...
    begin
      automatic logic [7:0] rd[256];
      automatic logic [7:0] status;
      // Flush TX FIFO: stub may have left data from earlier tests
      xfcp_write(32'hFF07_0004, 32'h2, 8'hA6); drain_write_resp();
      repeat (4) @(posedge clk_i);
    ...
    // T44: STREAM_WRITE sid=1 256B -> stub consume -> RX_LEVEL==0 + TX ERR\n -> drain
    // ==============================================================
    $display("--- T44: STREAM_WRITE sid=1 256B -> stub consume -> RX_LEVEL==0 + ERR\\n ---");
    begin
      automatic logic [7:0] wr[256];
      automatic logic [7:0] rd[256];
      automatic logic [7:0] status;
      ...
      // Stub consumed all bytes (cmd_len>4 -> not PING): RX_LEVEL==0
      xfcp_read(32'hFF07_0010, 8'hDD);
      recv_read(rdata_t44);
      chk32(rdata_t44, 32'h0, "T44.RX_LEVEL==0_stub_consumed");
      // Stub responded ERR\n: TX_LEVEL==4
      xfcp_read(32'hFF07_0014, 8'hDE);
      recv_read(rdata_t44);
      chk32(rdata_t44, 32'h4, "T44.TX_LEVEL==4_stub_ERR");
      // Drain stub response to leave TX FIFO clean for T45
      xfcp_stream_read(8'hDF, 8'h01, 16'd4);
      recv_stream_read(16'd4, rd, status);
      chk8(status, 8'h00, "T44.stream_read_status==OK");
      chk8(rd[0], 8'h45, "T44.rd[0]=='E'");
      chk8(rd[3], 8'h0A, "T44.rd[3]==LF");
    end
    ...
    // T46: STREAM_WRITE sid=1 8B -> stub consume -> RX_LEVEL==0 + TX ERR\n -> drain
    // ==============================================================
    $display("--- T46: STREAM_WRITE sid=1 8B -> stub consume -> RX_LEVEL==0 + ERR\\n ---");
    begin
      automatic logic [7:0] wr[256] = '{default: 8'h00};
      automatic logic [7:0] rd[256];
      ...
      repeat (200) @(posedge clk_i);
      // Stub consumed all bytes (8B, not PING): RX_LEVEL==0
      xfcp_read(32'hFF07_0010, 8'hE9); recv_read(rdata_t46);
      chk32(rdata_t46, 32'h0, "T46.RX_LEVEL==0_stub_consumed");
      // Stub responded ERR\n: TX_LEVEL==4
      xfcp_read(32'hFF07_0014, 8'hEA); recv_read(rdata_t46);
      chk32(rdata_t46, 32'h4, "T46.TX_LEVEL==4_stub_ERR");
      // Drain stub response to leave TX FIFO empty for T47/T48
      xfcp_stream_read(8'hEB, 8'h01, 16'd4);
      recv_stream_read(16'd4, rd, status);
      chk8(status, 8'h00, "T46.stream_read_status==OK");
      chk8(rd[0], 8'h45, "T46.rd[0]=='E'");
      chk8(rd[3], 8'h0A, "T46.rd[3]==LF");
    end
    ...
    // ==============================================================
    // T48: STATUS sanity: STREAM_WRITE 4B -> stub consumed -> STATUS.rx_not_empty==0 + TX flush
    // ==============================================================
    $display("--- T48: STREAM_WRITE 4B -> stub consumed -> STATUS.rx_not_empty==0 + TX flush ---");
    begin
      automatic logic [7:0] wr[256] = '{default: 8'h00};
      ...
      repeat (200) @(posedge clk_i);
      // Stub consumed data: STATUS[0]=rx_not_empty==0, STATUS[1]=rx_full==0
      xfcp_read(32'hFF07_0008, 8'hF3); recv_read(rdata_t48);
      chk8(rdata_t48[0], 1'b0, "T48.STATUS.rx_not_empty==0_stub_consumed");
      chk8(rdata_t48[1], 1'b0, "T48.STATUS.rx_full==0");
      // rx_flush is a no-op (already empty), but verify it doesn't break STATUS
      ...
      // Flush TX FIFO (stub ERR\n response) to leave clean state for T49
      xfcp_write(32'hFF07_0004, 32'h2, 8'hF5); drain_write_resp();
      repeat (4) @(posedge clk_i);
    end
    ...
    // ==============================================================
    // T50: STREAM_WRITE sid=1 "PING" (4B) -> stub FSM -> STREAM_READ "PONG"
    // ==============================================================
    $display("--- T50: STREAM_WRITE sid=1 PING -> stub FSM -> STREAM_READ PONG ---");
    begin
      automatic logic [7:0] wr_t50 [256];
      automatic logic [7:0] rd_t50 [256];
      automatic logic [7:0] status_t50;
      for (int i = 0; i < 256; i++) wr_t50[i] = 8'h00;
      wr_t50[0] = 8'h50; wr_t50[1] = 8'h49; wr_t50[2] = 8'h4E; wr_t50[3] = 8'h47;
      xfcp_stream_write(8'h01, 8'h01, 16'd4, wr_t50);
      drain_stream_write_resp(status_t50);
      chk8(status_t50, 8'h00, "T50.stream_write_status==OK");
      repeat (200) @(posedge clk_i);
      xfcp_stream_read(8'h02, 8'h01, 16'd4);
      recv_stream_read(16'd4, rd_t50, status_t50);
      chk8(status_t50, 8'h00, "T50.stream_read_status==OK");
      chk8(rd_t50[0], 8'h50, "T50.rd[0]=='P'");
      chk8(rd_t50[1], 8'h4F, "T50.rd[1]=='O'");
      chk8(rd_t50[2], 8'h4E, "T50.rd[2]=='N'");
      chk8(rd_t50[3], 8'h47, "T50.rd[3]=='G'");
    end

    // ==============================================================
    // T51: STREAM_WRITE sid=1 "ABCD" (4B) -> stub FSM -> STREAM_READ "ERR\n"
    // ==============================================================
    $display("--- T51: STREAM_WRITE sid=1 ABCD -> stub FSM -> STREAM_READ ERR\\n ---");
    begin
      automatic logic [7:0] wr_t51 [256];
      automatic logic [7:0] rd_t51 [256];
      automatic logic [7:0] status_t51;
      for (int i = 0; i < 256; i++) wr_t51[i] = 8'h00;
      wr_t51[0] = 8'h41; wr_t51[1] = 8'h42; wr_t51[2] = 8'h43; wr_t51[3] = 8'h44;
      xfcp_stream_write(8'h03, 8'h01, 16'd4, wr_t51);
      drain_stream_write_resp(status_t51);
      chk8(status_t51, 8'h00, "T51.stream_write_status==OK");
      repeat (200) @(posedge clk_i);
      xfcp_stream_read(8'h04, 8'h01, 16'd4);
      recv_stream_read(16'd4, rd_t51, status_t51);
      chk8(status_t51, 8'h00, "T51.stream_read_status==OK");
      chk8(rd_t51[0], 8'h45, "T51.rd[0]=='E'");
      chk8(rd_t51[1], 8'h52, "T51.rd[1]=='R'");
      chk8(rd_t51[2], 8'h52, "T51.rd[2]=='R'");
      chk8(rd_t51[3], 8'h0A, "T51.rd[3]==LF");
    end

    // ==============================================================
    // T52: PING->PONG x4 back-to-back repeated
    // ==============================================================
    $display("--- T52: PING->PONG x4 back-to-back ---");
    begin
      automatic logic [7:0] wr_t52 [256];
      automatic logic [7:0] rd_t52 [256];
      automatic logic [7:0] status_t52;
      for (int i = 0; i < 256; i++) wr_t52[i] = 8'h00;
      wr_t52[0] = 8'h50; wr_t52[1] = 8'h49; wr_t52[2] = 8'h4E; wr_t52[3] = 8'h47;
      for (int iter = 0; iter < 4; iter++) begin
        xfcp_stream_write(8'h05, 8'h01, 16'd4, wr_t52);
        drain_stream_write_resp(status_t52);
        chk8(status_t52, 8'h00, "T52.stream_write_status==OK");
        repeat (200) @(posedge clk_i);
        xfcp_stream_read(8'h06, 8'h01, 16'd4);
        recv_stream_read(16'd4, rd_t52, status_t52);
        chk8(status_t52, 8'h00, "T52.stream_read_status==OK");
        chk8(rd_t52[0], 8'h50, "T52.rd[0]=='P'");
        chk8(rd_t52[1], 8'h4F, "T52.rd[1]=='O'");
        chk8(rd_t52[2], 8'h4E, "T52.rd[2]=='N'");
        chk8(rd_t52[3], 8'h47, "T52.rd[3]=='G'");
      end
    end

    // ==============================================================
    // T53: STR0 sid=0 loopback isolation (stub active on sid=1)
    // ==============================================================
    $display("--- T53: STR0 sid=0 loopback isolation with stub active on sid=1 ---");
    begin
      automatic logic [7:0] wr_t53 [256];
      automatic logic [7:0] rd_t53 [256];
      automatic logic [7:0] status_t53;
      for (int i = 0; i < 256; i++) wr_t53[i] = 8'h00;
      wr_t53[0] = 8'hDE; wr_t53[1] = 8'hAD; wr_t53[2] = 8'hBE; wr_t53[3] = 8'hEF;
      xfcp_stream_write(8'h07, 8'h00, 16'd4, wr_t53);
      drain_stream_write_resp(status_t53);
      chk8(status_t53, 8'h00, "T53.stream_write_status==OK");
      xfcp_stream_read(8'h08, 8'h00, 16'd4);
      recv_stream_read(16'd4, rd_t53, status_t53);
      chk8(status_t53, 8'h00, "T53.stream_read_status==OK");
      chk8(rd_t53[0], 8'hDE, "T53.rd[0]==0xDE");
      chk8(rd_t53[1], 8'hAD, "T53.rd[1]==0xAD");
      chk8(rd_t53[2], 8'hBE, "T53.rd[2]==0xBE");
      chk8(rd_t53[3], 8'hEF, "T53.rd[3]==0xEF");
    end

    // ==============================================================
    // T54: Long payload 8B (PING+extra) -> stub -> ERR\n (cmd_len != 4)
    // ==============================================================
    $display("--- T54: STREAM_WRITE sid=1 8B (PING+4x) -> stub -> ERR\\n ---");
    begin
      automatic logic [7:0] wr_t54 [256];
      automatic logic [7:0] rd_t54 [256];
      automatic logic [7:0] status_t54;
      for (int i = 0; i < 256; i++) wr_t54[i] = 8'h00;
      wr_t54[0] = 8'h50; wr_t54[1] = 8'h49; wr_t54[2] = 8'h4E; wr_t54[3] = 8'h47;
      wr_t54[4] = 8'h78; wr_t54[5] = 8'h78; wr_t54[6] = 8'h78; wr_t54[7] = 8'h78;
      xfcp_stream_write(8'h09, 8'h01, 16'd8, wr_t54);
      drain_stream_write_resp(status_t54);
      chk8(status_t54, 8'h00, "T54.stream_write_status==OK");
      repeat (200) @(posedge clk_i);
      xfcp_stream_read(8'h0A, 8'h01, 16'd4);
      recv_stream_read(16'd4, rd_t54, status_t54);
      chk8(status_t54, 8'h00, "T54.stream_read_status==OK");
      chk8(rd_t54[0], 8'h45, "T54.rd[0]=='E'");
      chk8(rd_t54[1], 8'h52, "T54.rd[1]=='R'");
      chk8(rd_t54[2], 8'h52, "T54.rd[2]=='R'");
      chk8(rd_t54[3], 8'h0A, "T54.rd[3]==LF");
    end
