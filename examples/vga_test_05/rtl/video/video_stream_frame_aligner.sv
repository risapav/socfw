...
//
// EOL/EOF integrity checks use last_active_x_req_i / last_active_pixel_req_i,
// which are in the same timing phase as pixel_req_i (1 cycle before de_i).
// A mismatch between stream EOL/EOF metadata and expected position triggers
// sync_error_o.
//
...
module video_stream_frame_aligner #(
  ...
  input  logic de_i,
  input  logic last_active_x_req_i,      // last active column (pixel_req phase)
  input  logic last_active_pixel_req_i,  // last frame pixel (pixel_req phase)
  ...
  output logic sync_error_o,        // pulse: unexpected SOF/EOL/EOF mismatch
        ...
        if (pixel_req_i) begin
          if (s_axis_valid_i) begin
            ...
            if (s_axis_sof_i && !frame_start_i) begin
              ...
              // EOL/EOF integrity: stream metadata must match timing position
              if (s_axis_eol_i != last_active_x_req_i)
                sync_error_o = 1'b1;
              if (s_axis_eof_i != last_active_pixel_req_i)
                sync_error_o = 1'b1;
            end
          end else begin
