...
module tb_hdmi_period_scheduler;
  ...
  task automatic test_no_island;
    ...
    // With VIDEO_TRIG=10, trigger fires at blank_remaining=10.  Preamble
    // takes 8 cycles (blank_remaining=10..3), GB takes 2 cycles (blank_remaining=2,1).
    // Both GB cycles are visible inside the hblank loop.
    if (vid_gb == 2)
  ...
  // ST_CONTROL re-enters; blank_remaining=10 <= VIDEO_TRIG=10 → trigger fires.
