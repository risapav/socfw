 ...
 *
 * Changelog:
 *  [v160] T_WTR_CYCLES znizene z 2 na 1 -- SDR SDRAM pri CL=3 potrebuje
 *         len 1 cyklus Write-to-Read turnaround. Overene simulaciou v160.
 *  [v160] T_RDL_CYCLES = CAS_LATENCY + 1 (Read-to-Precharge)
 *  [v170] T_RFC_CYCLES 7->8, T_RAS_CYCLES 4->6 -- oprava pre 125 MHz (8 ns perioda)
 */
`ifndef SDRAM_PKG_SV
`define SDRAM_PKG_SV
...
// ===========================================================================
// 1. MEMORY GEOMETRY (Konfigurovatelne)
// ===========================================================================
...
// ===========================================================================
// 2. TIMING PARAMETERS (@125 MHz, 8 ns period, W9825G6KH-6)
// ===========================================================================
parameter int T_RCD_CYCLES  = 2;   // tRCD=15 ns  -> 15/8=1.875 -> 2  (bolo: 2)
parameter int T_RP_CYCLES   = 2;   // tRP =15 ns  -> 15/8=1.875 -> 2  (bolo: 2)
parameter int T_RFC_CYCLES  = 8;   // tRFC=60 ns  -> 60/8=7.5   -> 8  (bolo: 7 -- CHYBA @125MHz)
parameter int T_RAS_CYCLES  = 6;   // tRAS=42 ns  -> 42/8=5.25  -> 6  (bolo: 4 -- CHYBA @125MHz)
parameter int CAS_LATENCY   = 3;   // CAS Latency (CL)
parameter int T_WR_CYCLES   = 2;   // tWR =14 ns  -> 14/8=1.75  -> 2  (bolo: 2)
parameter int T_WTR_CYCLES  = 1;   // Write-to-Read turnaround (bolo: 2, znizene v160)
parameter int T_RDL_CYCLES  = CAS_LATENCY + 1;  // Read-to-Precharge
parameter int T_REFI_CYCLES = 780; // 64ms/8192=7812 ns; 780*8=6240 ns < 7812 ns (safe, bolo: 780)

// ===========================================================================
// 3. AXI -> SDRAM ADDRESS MAPPING
// ===========================================================================
// Priklad pre DATA_WIDTH=16, COL=9, BANK=2, ROW=13:
//   AXI addr [31:0]:
//   [0]      = byte offset (ignorovany, 16-bit = 2B zarovnanie)
//   [9:1]    = col  (9 bitov)
//   [11:10]  = bank (2 bity)
//   [24:12]  = row  (13 bitov)
//   [31:25]  = nevyuzite
...
/*
// Sanity check -- adresovy priestor musi sa zmestit do AXI_DATA_WIDTH bitov
// (ROW_BIT_HIGH + 1 <= AXI_DATA_WIDTH)
// synthesis translate_off
// V SystemVerilogu sa staticke overenie v baliku pise bez 'if'
// Ak je podmienka nepravdiva, kompilacia skonci chybou.
// Poznamka: ROW_BIT_HIGH + 1 nesmie prekrocit AXI_DATA_WIDTH (32)
...
/**
 * @struct sdram_addr_t
 * @brief Dekodovana adresa pre vnutornu logiku kontrolera.
 */
...
/**
 * @struct sdram_cmd_t
 * @brief Rozsireny prikaz pre Scheduler a Read Engine.
 * @details Obsahuje meta-data potrebne na udrzanie AXI kontextu cez pipeline.
 */
typedef struct packed {
  sdram_addr_t             addr;
  logic                    write_en;
  ...
  logic                    last;  // Posledny beat v burste
} sdram_cmd_t;
