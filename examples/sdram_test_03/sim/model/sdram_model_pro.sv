...
module sdram_model_pro #(
  ...
  logic                  dq_drive;
  logic                  dq_drive_d;  // tOH hold: DQ driven 1 extra cycle after BL=1

  assign dq = (dq_drive || dq_drive_d) ? dq_out : 'z;

  // ================================================================
  // CAS LATENCY PIPELINE
  // CL stupnov (indexy 0..CL-1).
  //
  // SIM-TIMING POZNAMKA (dolezite pre PHY alignment):
  //   READ_CAPTURE_SHIFT=-1 (READ_IDX=CL-1): PHY captures at posedge T_cmd+CL.
  //   SDRAM sees READ at T_cmd+1 (1 CLK_SH cycle after PHY registers CMD).
  //   cas_pipe[CL-1] fires NBA at T_cmd+CL = T_cmd+3.
  //   PHY captures PRE-NBA at T_cmd+CL = T_cmd+3 => DQ stable from prev NBA. OK.
  //
  //   dq_drive_d: 1 extra cyklus hold pre tOH sim modelovanie.
  //   dq_out zostava stabilny pocas dq_drive_d cyklu (no dq_out_d needed).
  // ================================================================
  ...
  always_ff @(posedge clk) begin
    ...
    if (rst) begin
      ...
      // JEDEC-correct: drive DQ from cas_pipe[CL-1] (N cycles after RD).
      // PHY captures 1 cycle AFTER this NBA (no race with SHIFT=0 and extra CLK125 FF).
