/**
 * @file xfcp_test_06_top.sv
 * @brief XFCP switch s dvoma nezavislymi vstupmi: UART a ETH-UDP. v0.9+STATUS.
 ...
 *
 *   Architektura rovnaka ako xfcp_test_05_top, zmeny:
 *     - xfcp_pkg rozsireneny o xfcp_status_e
 *     - Response format: 4-bajtovy header (SOP+TYPE+SEQ+STATUS)
 *       namiesto 21-bajtoveho (SOP+TYPE+SEQ+DEV_TYPE+DEV_STR)
 *     - AXI BRESP/RRESP propagovane do STATUS byte
 *     - ID_STR parameter odstraneny z xfcp_fabric_endpoint
 *
...
`ifndef XFCP_TEST_06_TOP_SV
`define XFCP_TEST_06_TOP_SV
...
`endif
  ...
  // Skid buffer: cuts resp_buf -> eth_tx_arb|m_tdata_q timing path
  ...
  // Pipeline register: breaks sync_fifo.tail_q -> xfcp_fifo.mem (~11.9 ns)
