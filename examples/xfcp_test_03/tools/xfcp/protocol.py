...
RESP_HEADER  = 20   # SOP(1) + TYPE(1) + DEV_TYPE(2) + DEV_STR(16)
...
def encode_read(addr: int, num_words: int) -> bytes:
    return (
        bytes([SOP_REQ, OP_READ])
        + struct.pack(">H", num_words * 4)
        + struct.pack(">I", addr)
    )


def encode_write(addr: int, values: list) -> bytes:
    payload = struct.pack(f">{len(values)}I", *values)
    return (
        bytes([SOP_REQ, OP_WRITE])
...
def decode_read_response(raw: bytes, num_words: int) -> list:
...
def decode_write_response(raw: bytes) -> None:
