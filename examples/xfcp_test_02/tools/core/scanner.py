...
class DynamicScanner:
    ...
    Maps 4-char ASCII identifiers to Python peripheral classes.
    """
    ...
    def __init__(self, bus, base_io=0xFF000000, stride=0x10000,
        ...
        self.slot_retries = slot_retries   # scan-level retries (e.g. FPGA still booting)
    ...
    def scan(self):
        ...
        for slot in range(self.num_slots):
            ...
            if addr in self.blacklist:
                print(f"  [Slot {slot:X}] {Fore.LIGHTBLACK_EX}SKIP{Style.RESET_ALL}")
                continue

            dev_id_raw = self._read_slot(addr)

            if dev_id_raw is None:
                print(f"  [Slot {slot:X}] {Fore.RED}TIMEOUT{Style.RESET_ALL} "
                      f"@ {hex(addr)}")
                continue
            ...
            try:
                ...
                print(f"  [Slot {slot:X}] {Fore.LIGHTBLACK_EX}non-ASCII "
                      f"({hex(dev_id_raw)}){Style.RESET_ALL}")

        return inventory

    def _read_slot(self, addr):
        """Read slot ID with scan-level retries (helps if FPGA is still booting)."""
