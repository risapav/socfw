// tb_uart_fifo_loopback_os -- integration test: uart_fifo_os loopback
//
// Tests:
...
//   BIT_CLKS   = 128
//   OS_PRESCALE = round(1_600_000 / (12_500 * 16)) = 8
//   16 OS ticks x 8 clks = 128 clks/bit (exact)
//   FIFO_DEPTH  = 16 (small for faster simulation)
//
// Run: vsim -c -do "run -all; quit" tb_uart_fifo_loopback_os

`timescale 1ns/1ps

module tb_uart_fifo_loopback_os;
  import uart_pkg::*;
  ...
  // =========================================================================
  // DUT: uart_fifo_os (16x oversampled RX)
  // =========================================================================
  uart_fifo_os #(
  ...
  // =========================================================================
  // BFM: capture echo byte from txd
  // =========================================================================
  ...
  initial begin
    ...
    repeat(4) @(posedge clk);
    if (fails == 0)
      $display("PASS tb_uart_fifo_loopback_os: all tests passed");
    else
      $display("FAIL tb_uart_fifo_loopback_os: %0d test(s) failed", fails);
