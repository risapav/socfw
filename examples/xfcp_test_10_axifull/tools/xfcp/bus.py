...
class XfcpBus:
    ...
    def get_target_info(self, index: int) -> dict | None:
        """GET_TARGET_INFO (op=0x03): vrati target dict alebo None pri chybe."""
        seq = self._next_seq()
        pkt = proto.encode_get_target_info(index, seq=seq)
        try:
            raw = self._transact(pkt, proto.resp_len_get_target_info())
            return proto.decode_get_target_info_response(raw, expected_seq=seq)
        except (XfcpTimeoutError, XfcpProtocolError, XfcpStatusError):
            return None

    def list_targets(self, max_index: int = 255) -> list:
        """Vrati zoznam vsetkych targetov (GET_TARGET_INFO index 0..N-1).

        Skenuje od indexu 0 a zastavi pri prvom BAD_ADDRESS.
        """
        results = []
        for i in range(max_index + 1):
            seq = self._next_seq()
            pkt = proto.encode_get_target_info(i, seq=seq)
            try:
                raw = self._transact(pkt, proto.resp_len_get_target_info())
                info = proto.decode_get_target_info_response(raw, expected_seq=seq)
                results.append(info)
            except XfcpStatusError as e:
                if e.status == 0x03:  # BAD_ADDRESS -> koniec tabulky
                    break
                results.append(None)
