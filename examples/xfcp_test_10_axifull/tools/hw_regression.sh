#!/usr/bin/env bash
# hw_regression.sh — xfcp_test_09_targets HW regresia: UART + UDP
#
# Pouzitie:
...
echo ""
echo "=========================================="
echo "  xfcp_test_09_targets HW regresia"
...
run_test "UART transport ($UART_PORT @ $BAUD)" \
    --uart "$UART_PORT" --baud "$BAUD" --caps --targets --rw --stream --diag

run_test "UDP  transport ($FPGA_IP:$UDP_PORT)" \
    --udp  "$FPGA_IP:$UDP_PORT" --caps --targets --rw --stream --diag
