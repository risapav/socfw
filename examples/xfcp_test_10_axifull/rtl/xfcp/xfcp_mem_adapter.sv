...
module xfcp_mem_adapter #(
  ...
  localparam int RFIFO_AW   = $clog2(BEATS_MAX);
  ...
  // Read data capture FIFO (BEATS_MAX entries — pokryva plny burst 64 beatov).
  // Nutne: rfifo musi pojat cely burst kym endpoint zacne konzumovat (ARB_WAIT_PKT).
  // 2-entry FIFO by sposobilo deadlock pre bursts > 2 beatov:
  //   rfifo plny → m_axi_rready=0 → SRAM caka → rlast nikdy → ST_DONE_PLS nikdy.
  logic [DATA_WIDTH-1:0]   rfifo_data_q [0:BEATS_MAX-1];
  logic [1:0]              rfifo_resp_q [0:BEATS_MAX-1];
  logic [BEATS_MAX-1:0]    rfifo_last_q;
  logic [RFIFO_AW-1:0]     rfifo_wr_ptr_q, rfifo_rd_ptr_q;
  logic [RFIFO_AW:0]       rfifo_cnt_q;
  wire  rfifo_full_w  = (rfifo_cnt_q == (RFIFO_AW+1)'(BEATS_MAX));
  wire  rfifo_empty_w = (rfifo_cnt_q == '0);

  wire rfifo_push_w = m_axi_rvalid && m_axi_rready && !rfifo_full_w;
  wire rfifo_pop_w  = mem_rdata_valid_o && mem_rdata_ready_i && !rfifo_empty_w;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (rfifo_push_w) begin
        rfifo_data_q[rfifo_wr_ptr_q] <= m_axi_rdata;
        rfifo_resp_q[rfifo_wr_ptr_q] <= m_axi_rresp;
        rfifo_last_q[rfifo_wr_ptr_q] <= m_axi_rlast;
        rfifo_wr_ptr_q               <= rfifo_wr_ptr_q + RFIFO_AW'(1);
      end
      if (rfifo_pop_w)
        rfifo_rd_ptr_q <= rfifo_rd_ptr_q + RFIFO_AW'(1);
      case ({rfifo_push_w, rfifo_pop_w})
        2'b10:   rfifo_cnt_q <= rfifo_cnt_q + (RFIFO_AW+1)'(1);
        2'b01:   rfifo_cnt_q <= rfifo_cnt_q - (RFIFO_AW+1)'(1);
        default: ;
      endcase
    end
  end
      ...
      case (state_q)
        ...
        ST_R: begin
          if (timeout_w) begin
            ...
            to_cnt_q <= to_cnt_q + 1'b1;
            if (m_axi_rvalid && m_axi_rready && !rfifo_full_w) begin
              to_cnt_q <= '0;
        ...
        ST_DATA: begin
          // Streamujeme rfifo -> rdata; skoncime ked rfifo_empty po poslednom beate
          if (mem_rdata_valid_o && mem_rdata_ready_i) begin
            if (rfifo_last_q[rfifo_rd_ptr_q] && !rfifo_empty_w)
              state_q <= ST_IDLE;
          end
        end
  ...
  // ── AXI R channel ────────────────────────────────────────────────────────
  assign m_axi_rready  = (state_q == ST_R) && !rfifo_full_w && !timeout_w;
