 ...
 * @brief Pipelined XFCP multi-device endpoint -- AXIL + AXIS + CAPS + TI (A-light routing).
 *
 * @details
 ...
 *   a jednym TARGET-INFO adapterom (xfcp_target_info_adapter).
 ...
 *
 *   Rozsirenia oproti xfcp_test_08_caps (TI extension):
 *     1. order_entry_t: is_ti flag pridany vedla is_axis, is_caps
 *     2. dec_valid: || is_ti_op
 *     3. eng_busy/eng_done_cnt: !is_ti_op guard pridany
 *     4. ti_done_cnt_q: analogicky k caps_done_cnt_q
 *     5. ti_req_valid_o dispatch (okamzita odpoved, ziadny busy tracker)
 *     6. rdata MUX: arb_is_ti_q -> ti_rdata_i
 *     7. ti_rdata_ready_o: analogicky k caps_rdata_ready_o
 *     8. Arbiter done MUX: is_ti ? ti_done_rdy
 *     9. arb_is_axil_q: preregistered !(axis || caps || ti)
 */
...
module xfcp_fabric_endpoint #(
  ...
  // order_entry_t: is_axis=AXIS, is_caps=CAPS, is_ti=TARGET_INFO, else AXIL.
  ...
  // ── Address decoder ──────────────────────────────────────────
  // STREAM/CAPS/TI ops bypass address decode.
  ...
  assign dec_valid = dec_valid_axil || is_stream_op || is_caps_op || is_ti_op;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      for (int i = 0; i < NUM_SLAVES; i++) begin
        // !is_stream_op, !is_caps_op, !is_ti_op: prevent non-AXIL ops from setting eng_busy.
        if (req_valid && req_ready && !invalid_req && !is_stream_op
            && !is_caps_op && !is_ti_op && dec_sel == SEL_W'(i))
          eng_busy[i] <= 1'b1;
  ...
  logic          arb_is_caps_q;
  logic          arb_is_ti_q;
  logic          arb_is_axil_q;  // preregistered: !(axis || caps || ti)
  ...
  // rdata MUX: AXIL engine vs AXIS adapter vs CAPS adapter vs TI adapter.
  always_comb begin
  ...
  always_ff @(posedge clk) begin
    if (rst_n) begin
      ...
      if (resp_start_q)
        $display("[%0t] %m: RESP is_axis=%0b is_caps=%0b is_ti=%0b sel=%0d type=0x%02h",
                 $time, arb_is_axis_q, arb_is_caps_q, arb_is_ti_q,
                 arb_sel_q, 8'(resp_type_q));
      if (arb_q != arb_n)
        $display(
          "[%0t] %m: ARB %0d->%0d sel=%0d is_axis=%0b is_caps=%0b is_ti=%0b head_v=%0b",
          $time, arb_q, arb_n, arb_sel_q, arb_is_axis_q, arb_is_caps_q,
          arb_is_ti_q, ofifo_head_valid_r);
    end
  end
  // synthesis translate_on
