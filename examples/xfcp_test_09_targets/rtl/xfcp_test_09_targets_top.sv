/**
 * @file xfcp_test_09_targets_top.sv
 * @brief XFCP switch: UART + ETH-UDP, v1.2+CAPS+AXIS+TARGETS.
 ...
 *
 *   Rozsirenia oproti xfcp_test_08_caps_top:
 *     - xfcp_pkg: GET_TARGET_INFO opcode (0x03) + RESP_GET_TARGET_INFO (0x04)
 *     - xfcp_fabric_endpoint: TI routing (4. backend vedla AXIL + AXIS + CAPS)
 ...
 *     Byte 0: proto_major=1  Byte 1: proto_minor=2
 *     Byte 2: num_axil=7     Byte 3: num_stream=1
 *     Byte 4-5: max_stream=256 (0x0100 big-endian)
 *     Byte 6: stream_align=4  Byte 7: caps_flags=0x0F (AXIL|STREAM|CAPS|TARGETS)
 *
 *   GET_TARGET_INFO tabulka (index 0..7):
 ...
 *     7: STREAM STR0 base=0 max=256B align=4
 */

`ifndef XFCP_TEST_09_TARGETS_TOP_SV
`define XFCP_TEST_09_TARGETS_TOP_SV
...
`endif
  ...
  xfcp_caps_adapter #(
    ...
    .MAX_STREAM_BYTES (256),
    .STREAM_ALIGN     (4),
    .CAPS_FLAGS       (4'b1111)  // AXIL | STREAM | CAPS | TARGETS
  ) u_caps_adapter (
    .clk              (clk_i), .rst_n (rst_ni),
  ...
  // ── XFCP TARGET_INFO Adapter (GET_TARGET_INFO opcode 0x03) ────
  xfcp_target_info_adapter #(
    ...
    .TARGET_MAX_XFER ('{
      ...
      16'd128, 16'd128, 16'd128, 16'd256
    }),
    .TARGET_ALIGN ('{
...
`endif // XFCP_TEST_09_TARGETS_TOP_SV
