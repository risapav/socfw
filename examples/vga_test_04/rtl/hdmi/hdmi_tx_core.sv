...
//
// Inputs:  RGB888 + DE + HSYNC + VSYNC (timed video stream)
// Outputs: 3 × 10-bit TMDS words per pixel clock (ch0=B, ch1=G, ch2=R)
//          Ready for a vendor PHY / ALTDDIO_OUT / OSERDES.
//
// Pipeline (all stages in pix_clk_i domain):
//   Cycle N   : rgb_i / de_i / hsync_i / vsync_i presented
//   Cycle N+1 : period_scheduler decides period; encoders see inputs
...
module hdmi_tx_core #(
  ...
  // Timed video stream (from video_stream_frame_aligner / vga path)
  ...
  // Derive hblank / vblank from registered signals.
  // vblank tracks the vsync period; hblank is any non-DE time outside vblank.
  wire vblank = vsync_r;
  ...
  // de_r is passed so the encoder can reset running disparity during blanking.
  ...
  // Use {vsync_r, hsync_r} directly — no extra delay register needed.
  // Pipeline analysis: ctrl_r[T+2]=f(vsync_r[T+1])=f(vsync_i[T]), matching
  // video encoder which produces tmds_o[T+3] for red_r[T+1]=red_i[T].
  // Both arrive at the mux in the same cycle when driven by period_d1.
  ...
  // period_o (from scheduler) is registered 1 cycle after de_r changes.
  // Encoders have LATENCY=2 from red_r/de_r.  With 1 delay stage:
  //   period_d1 during cycle T+3 = f(de_r[T+1]) = f(de_i[T])
  //   video_ch* during cycle T+3 = encoded(red_r[T+1]) = encoded(red_i[T])
  //   ctrl_ch*  during cycle T+3 = LUT(vsync_r[T+1])  = LUT(vsync_i[T])
  // All three inputs represent the same source cycle T → mux is aligned.
  ...
  if (ENABLE_DATA_ISLAND) begin : gen_data_island
    ...
    // Packet scheduler (triggered once per frame on vsync edge)
    logic vsync_prev;
    logic eof_pulse;
    ...
    // TERC4 encoders: nibble mapping (simplified — ch0 lower nibble, etc.)
