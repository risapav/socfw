/**
 * @file    xfcp_pkg.sv
 * @brief   Definicia struktur a opkodov protokolu XFCP v0.9+STATUS+AXIS.
...
package xfcp_pkg;
  ...
  // Mapovanie request opcode -> response opcode.
  ...
  function automatic logic xfcp_resp_has_payload(input xfcp_op_e op);
    return (op == XFCP_OP_RESP_READ) || (op == XFCP_OP_RESP_STREAM_READ);
  endfunction
