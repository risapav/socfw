...
module tb_sdram_top;
  ...
  always @(posedge sdram_clk) begin
    if (!sdram_cs_n) begin
      case ({sdram_ras_n, sdram_cas_n, sdram_we_n})
        ...
        3'b010: $display("[%0t] PRE %s B=%0h", $time, sdram_addr[10] ? "ALL" : "   ", sdram_ba);
  ...
  initial begin
    ...
    // ------------------------------------------------------------------
    // ------------------------------------------------------------------
    // TEST 8: Bank Pipeline — meranie skutočného zisku v160
    // Všetky 4 zápisy do FIFO pred čakaním na B response.
    // Scheduler dostane plné FIFO → aktivuje banky paralelne.
    // ------------------------------------------------------------------
    $display("\n--- Test 8: Bank Pipeline ---");
    begin
      automatic logic [31:0] pipe_addrs[4] = '{
        32'h0010_0000,   // B=0, Row=0x100
        32'h0010_0400,   // B=1, Row=0x100
        32'h0010_0800,   // B=2, Row=0x100
        32'h0010_0C00    // B=3, Row=0x100
      };
      automatic logic [31:0] pipe_data[4] = '{
        32'hB0B0_B0B0,
        32'hB1B1_B1B1,
        32'hB2B2_B2B2,
        32'hB3B3_B3B3
      ...
      // Fáza 1: Všetky 4 AW+W bez čakania na B response
      for (int i = 0; i < 4; i++) begin
        @(posedge clk);
        s_axi_awaddr <= pipe_addrs[i];
        s_axi_awlen  <= 8'd0;
        s_axi_awid   <= 4'(i);
        @(posedge clk);
        s_axi_awvalid <= 1'b1;
        ...
        s_axi_wvalid <= 1'b0;
        s_axi_wlast  <= 1'b0;
        scoreboard[pipe_addrs[i]] = pipe_data[i];
        $display("[TB] PipeWrite %0d Addr=%08h Data=%08h",
                 i, pipe_addrs[i], pipe_data[i]);
      end

      // Fáza 2: Zober všetky 4 B responses
      repeat(4) begin
        s_axi_bready <= 1'b1;
        wait(s_axi_bvalid);
        @(posedge clk);
      ...
      s_axi_bready <= 1'b0;
      $display("[TB] All writes done at %0t ns (from start: %0t ns)",
               $time/1000, ($time - t_start)/1000);

      // Fáza 3: Čítaj všetky 4 adresy
      // Poznámka: read_addr_q sa plní cez AR monitor (always_ff)
      for (int i = 0; i < 4; i++) begin
        @(posedge clk);
        s_axi_araddr  <= pipe_addrs[i];
        s_axi_arlen   <= 8'd0;
        s_axi_arid    <= 4'(i);
        @(posedge clk);
        s_axi_arvalid <= 1'b1;
      ...
      $display("[TB] Pipeline total: %0t ns", ($time - t_start)/1000);
    end
    #100;
...
// ============================================================
// Vložiť do tb_sdram_top.sv pred záverečný "endmodule"
// ============================================================
`ifdef __DO_COVERAGE__
// Na túto funkčnosť je nutná QUESTA LICENCIA a tú nemám !!

// Coverage modul — sleduje interné signály cez hierarchiu
sdram_coverage u_coverage (
  .clk            (clk),
  .rstn           (rstn),

  // Scheduler interné signály
  .sched_state    (dut.i_scheduler.state),
  .refresh_pending(dut.i_scheduler.refresh_pending),
  .pipe_valid     (dut.i_scheduler.pipe_valid),

  // Bank status
  .bank_open      (dut.bank_open),
  .bank_row_hit   (dut.bank_row_hit),
  .bank_do_act    (dut.bank_do_act),
  .bank_do_pre    (dut.bank_do_pre),

  // Scheduler výstup
  .sched_valid    (dut.sched_valid),
  .sched_cmd      (dut.sched_cmd),
  .fifo_bank      (dut.fifo_cmd.addr.bank),
...
// Questa: zobraziť coverage report po simulácii:
//   vsim> coverage report -detail -cvg -assert
//   vsim> coverage report -html -output coverage_report/
`endif
