 ...
 * GMII TX outputs (gmii_txd_o, gmii_tx_en_o) are registered (output FF) to meet
 * PHY setup time. CRC input uses combinational path gmii_txd_comb_w — same byte
 * that the output FF will present to the PHY next cycle, so FCS remains correct.
...
module gmii_tx_mac #(
  ...
  localparam int IFG_BYTES      = 13; // +1: output register holds last FCS byte during IFG[0]
  ...
  logic [7:0]  gmii_txd_comb_w;
  logic        gmii_tx_en_comb_w;
  ...
  always_comb begin
    ...
    fcs_en            = (state_q == ST_ETH_HEADER) || (state_q == ST_PAYLOAD) ||
                        (state_q == ST_PADDING);
    gmii_tx_en_comb_w = (state_q != ST_IDLE) && (state_q != ST_IFG);
    gmii_txd_comb_w   = 8'h00;
    ...
    case (state_q)
      ...
      ST_FCS: begin
        unique case (fcs_cnt_q)
          2'd0: gmii_txd_comb_w = fcs_val[7:0];
          2'd1: gmii_txd_comb_w = fcs_val[15:8];
          2'd2: gmii_txd_comb_w = fcs_val[23:16];
          2'd3: gmii_txd_comb_w = fcs_val[31:24];
        endcase
  ...
  // Output registers: stabilise TXD/TX_EN one full cycle before PHY samples them.
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      gmii_txd_o   <= 8'h00;
      gmii_tx_en_o <= 1'b0;
      ...
      gmii_txd_o   <= gmii_txd_comb_w;
      gmii_tx_en_o <= gmii_tx_en_comb_w;
      gmii_tx_er_o <= 1'b0;
    end
  end
