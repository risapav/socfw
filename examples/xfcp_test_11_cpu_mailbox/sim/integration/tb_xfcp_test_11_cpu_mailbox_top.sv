...
// tb_xfcp_test_11_cpu_mailbox_top -- integration test for xfcp_test_11_cpu_mailbox_top.
//
...
// T17:     STREAM_WRITE sid=1 (CPU0 mailbox) 4B + STREAM_READ sid=1 verify.
// T18:     STREAM_WRITE sid=1 (CPU0 mailbox) 16B + STREAM_READ sid=1 verify.
...
// T23-T25: GET_CAPS (proto_minor=3, num_stream=2, caps_flags=0x1F).
...
// T29:     GET_TARGET_INFO index=10 (invalid) -> BAD_ADDRESS.
...
// T38:     STREAM_WRITE sid=1 (CPU0) 256B + STREAM_READ sid=1 (max mailbox).
...
// T42:     Alternating: STR0 write/read, CPU0 write/read, STR0 write/read.

module tb_xfcp_test_11_cpu_mailbox_top;
  import tb_eth_pkg::*;
  ...
  initial begin
    ...
    // T17: STREAM_WRITE sid=1 (CPU0 mailbox) 4B + STREAM_READ sid=1 verify
    $display("--- T17: STREAM sid=1 (CPU0) 4B loopback ---");
    begin
      automatic logic [7:0] wr[256] = '{default: 8'h00};
      automatic logic [7:0] rd[256];
      automatic logic [7:0] status;
      ...
      drain_stream_write_resp(status);
      chk8(status, 8'h00, "T17.write_status==OK");
      xfcp_stream_read(8'h71, 8'h01, 16'd4);
      recv_stream_read(16'd4, rd, status);
      chk8(status, 8'h00, "T17.read_status==OK");
      chk8(rd[0], 8'hDE, "T17.byte[0]");
      chk8(rd[1], 8'hAD, "T17.byte[1]");
      chk8(rd[2], 8'hBE, "T17.byte[2]");
      chk8(rd[3], 8'hEF, "T17.byte[3]");
    end

    // T18: STREAM_WRITE sid=1 (CPU0) 16B + STREAM_READ sid=1 verify
    $display("--- T18: STREAM sid=1 (CPU0) 16B loopback ---");
    begin
      automatic logic [7:0] wr[256] = '{default: 8'h00};
      automatic logic [7:0] rd[256];
      automatic logic [7:0] status;
      for (int i = 0; i < 16; i++) wr[i] = 8'(8'hC0 + i);
      xfcp_stream_write(8'h80, 8'h01, 16'd16, wr);
      drain_stream_write_resp(status);
      chk8(status, 8'h00, "T18.write_status==OK");
      xfcp_stream_read(8'h81, 8'h01, 16'd16);
      recv_stream_read(16'd16, rd, status);
      chk8(status, 8'h00, "T18.read_status==OK");
      for (int i = 0; i < 16; i++)
        chk8(rd[i], 8'(8'hC0 + i), $sformatf("T18.byte[%0d]", i));
    end
    ...
    // ==============================================================
    // T29: GET_TARGET_INFO index=10 (invalid, NUM_TARGETS=10) -> BAD_ADDRESS
    // ==============================================================
    $display("--- T29: GET_TARGET_INFO index=10 (invalid) -> BAD_ADDRESS ---");
    begin
      automatic logic [7:0] ti[16];
      automatic logic [7:0] status;
      xfcp_get_target_info(8'hD3, 8'h0A);
      recv_get_target_info(ti, status);
    ...
    // ==============================================================
    // T38: STREAM sid=1 (CPU0) 256B loopback (max)
    // ==============================================================
    $display("--- T38: STREAM sid=1 (CPU0) 256B loopback (max) ---");
    begin
      automatic logic [7:0] wr[256];
      automatic logic [7:0] rd[256];
      automatic logic [7:0] status;
      for (int i = 0; i < 256; i++) wr[i] = 8'(i ^ 8'h5A);
      xfcp_stream_write(8'hA0, 8'h01, 16'd256, wr);
      drain_stream_write_resp(status);
      chk8(status, 8'h00, "T38.write_status==OK");
      xfcp_stream_read(8'hA1, 8'h01, 16'd256);
      recv_stream_read(16'd256, rd, status);
      chk8(status, 8'h00, "T38.read_status==OK");
      for (int i = 0; i < 256; i++)
        if (rd[i] !== 8'(i ^ 8'h5A)) begin
          $display("FAIL T38.byte[%0d]: got=0x%02X exp=0x%02X", i, rd[i], i ^ 8'h5A);
          fails++;
        end
      $display("PASS T38 CPU0 256B loopback OK");
    end
    ...
    // ==============================================================
    // T42: Alternating STR0 / CPU0 / STR0 (stream mux integrity)
    // ==============================================================
    $display("--- T42: Alternating STR0/CPU0/STR0 mux integrity ---");
    begin
      automatic logic [7:0] wr[256] = '{default: 8'h00};
      automatic logic [7:0] rd[256];
      automatic logic [7:0] status;
      ...
      xfcp_stream_write(8'hD0, 8'h00, 16'd8, wr);
      drain_stream_write_resp(status);
      chk8(status, 8'h00, "T42.str0_write_status");
      // Write 8B to CPU0
      wr[0]=8'hAA; wr[1]=8'hBB; wr[2]=8'hCC; wr[3]=8'hDD;
      wr[4]=8'hEE; wr[5]=8'hFF; wr[6]=8'h01; wr[7]=8'h02;
      xfcp_stream_write(8'hD1, 8'h01, 16'd8, wr);
      drain_stream_write_resp(status);
      chk8(status, 8'h00, "T42.cpu0_write_status");
      // Read 8B from STR0 -- verify STR0 data not mixed with CPU0
      xfcp_stream_read(8'hD2, 8'h00, 16'd8);
      ...
      // Read 8B from CPU0 -- verify CPU0 data is intact
      xfcp_stream_read(8'hD3, 8'h01, 16'd8);
      recv_stream_read(16'd8, rd, status);
      chk8(status, 8'h00, "T42.cpu0_read_status");
      chk8(rd[0], 8'hAA, "T42.cpu0[0]");
      chk8(rd[7], 8'h02, "T42.cpu0[7]");
      $display("PASS T42 STR0/CPU0 isolation OK");
    end
