/**
 * @file xfcp_test_11_cpu_mailbox_top.sv
 * @brief XFCP switch: UART + ETH-UDP, v1.3+CAPS+AXIS+TARGETS+MEM + 2-stream mailbox.
 ...
 *
 *   Rozsirenia oproti xfcp_test_10_axifull_top:
 *     - xfcp_stream_mux: 2-way dispatch na 2 xfcp_axis_adapter instancie
 *     - xfcp_axis_adapter[0] (STREAM_ID=0): STR0 loopback FIFO (zachovany)
 *     - xfcp_axis_adapter[1] (STREAM_ID=1): CPU0 mailbox loopback FIFO
 ...
 *
 *   STREAM sloty:
 *     stream_id=0 (STR0): loopback FIFO 256 entries
 *     stream_id=1 (CPU0): mailbox loopback FIFO 256 entries (simulovany mailbox)
 *
 ...
 *     7: STREAM STR0 base=0x00000000 (sid=0) max=256B
 *     8: MEM   MEM0  base=0x00000000 max=256B (axifull_sram 1KB)
 *     9: STREAM CPU0 base=0x00000001 (sid=1) max=256B
 */

`ifndef XFCP_TEST_11_CPU_MAILBOX_TOP_SV
`define XFCP_TEST_11_CPU_MAILBOX_TOP_SV
...
`endif
  ...
  // ── Stream mux wires (fabric -> mux -> adp0 / adp1) ──────────
  logic          mux_a0_req_valid_w, mux_a0_req_ready_w;
  xfcp_req_hdr_t mux_a0_req_hdr_w;
  logic [31:0]   mux_a0_wdata_w;
  logic          mux_a0_wdata_valid_w, mux_a0_wdata_ready_w;
  logic [31:0]   mux_a0_rdata_w;
  logic          mux_a0_rdata_valid_w, mux_a0_rdata_ready_w;
  logic          mux_a0_resp_done_w;
  logic [7:0]    mux_a0_resp_status_w;

  logic          mux_a1_req_valid_w, mux_a1_req_ready_w;
  xfcp_req_hdr_t mux_a1_req_hdr_w;
  logic [31:0]   mux_a1_wdata_w;
  logic          mux_a1_wdata_valid_w, mux_a1_wdata_ready_w;
  logic [31:0]   mux_a1_rdata_w;
  logic          mux_a1_rdata_valid_w, mux_a1_rdata_ready_w;
  logic          mux_a1_resp_done_w;
  logic [7:0]    mux_a1_resp_status_w;

  xfcp_stream_mux u_stream_mux (
    .clk   (clk_i), .rst_n (rst_ni),
    // fabric side
    .fab_req_valid_i   (axis_req_valid_w),
    .fab_req_ready_o   (axis_req_ready_w),
    .fab_req_hdr_i     (axis_req_hdr_w),
    .fab_wdata_i       (axis_wdata_w),
    .fab_wdata_valid_i (axis_wdata_valid_w),
    .fab_wdata_ready_o (axis_wdata_ready_w),
    .fab_rdata_o       (axis_rdata_w),
    .fab_rdata_valid_o (axis_rdata_valid_w),
    .fab_rdata_ready_i (axis_rdata_ready_w),
    .fab_resp_done_o   (axis_resp_done_w),
    .fab_resp_status_o (axis_resp_status_w),
    // adapter 0 (STR0, stream_id=0)
    .a0_req_valid_o    (mux_a0_req_valid_w),
    .a0_req_ready_i    (mux_a0_req_ready_w),
    .a0_req_hdr_o      (mux_a0_req_hdr_w),
    .a0_wdata_o        (mux_a0_wdata_w),
    .a0_wdata_valid_o  (mux_a0_wdata_valid_w),
    .a0_wdata_ready_i  (mux_a0_wdata_ready_w),
    .a0_rdata_i        (mux_a0_rdata_w),
    .a0_rdata_valid_i  (mux_a0_rdata_valid_w),
    .a0_rdata_ready_o  (mux_a0_rdata_ready_w),
    .a0_resp_done_i    (mux_a0_resp_done_w),
    .a0_resp_status_i  (mux_a0_resp_status_w),
    // adapter 1 (CPU0, stream_id=1)
    .a1_req_valid_o    (mux_a1_req_valid_w),
    .a1_req_ready_i    (mux_a1_req_ready_w),
    .a1_req_hdr_o      (mux_a1_req_hdr_w),
    .a1_wdata_o        (mux_a1_wdata_w),
    .a1_wdata_valid_o  (mux_a1_wdata_valid_w),
    .a1_wdata_ready_i  (mux_a1_wdata_ready_w),
    .a1_rdata_i        (mux_a1_rdata_w),
    .a1_rdata_valid_i  (mux_a1_rdata_valid_w),
    .a1_rdata_ready_o  (mux_a1_rdata_ready_w),
    .a1_resp_done_i    (mux_a1_resp_done_w),
    .a1_resp_status_i  (mux_a1_resp_status_w)
  );

  // ── STR0: AXI-Stream loopback FIFO (stream_id=0) ─────────────
  logic [7:0] lb0_m_tdata_w;
  logic       lb0_m_tvalid_w, lb0_m_tready_w, lb0_m_tlast_w;
  logic [8:0] lb0r_rdata_w;
  logic       lb0r_rvalid_w, lb0r_rready_w;

  xfcp_fifo_reg #(.DATA_WIDTH(9), .DEPTH(256)) u_lb_str0 (
    .clk    (clk_i), .rst_n (rst_ni), .flush (1'b0),
    .w_valid(lb0_m_tvalid_w),
    .w_data ({lb0_m_tlast_w, lb0_m_tdata_w}),
    .w_ready(lb0_m_tready_w),
    .r_valid(lb0r_rvalid_w),
    .r_data (lb0r_rdata_w),
    .r_ready(lb0r_rready_w)
  );
  ...
  xfcp_axis_adapter #(
    ...
    .clk(clk_i), .rst_n(rst_ni),
    .axis_req_valid_i   (mux_a0_req_valid_w),
    .axis_req_ready_o   (mux_a0_req_ready_w),
    .axis_req_hdr_i     (mux_a0_req_hdr_w),
    .axis_wdata_i       (mux_a0_wdata_w),
    .axis_wdata_valid_i (mux_a0_wdata_valid_w),
    .axis_wdata_ready_o (mux_a0_wdata_ready_w),
    .axis_rdata_o       (mux_a0_rdata_w),
    .axis_rdata_valid_o (mux_a0_rdata_valid_w),
    .axis_rdata_ready_i (mux_a0_rdata_ready_w),
    .axis_resp_done_o   (mux_a0_resp_done_w),
    .axis_resp_status_o (mux_a0_resp_status_w),
    .m_axis_tdata_o  (lb0_m_tdata_w),
    .m_axis_tvalid_o (lb0_m_tvalid_w),
    .m_axis_tready_i (lb0_m_tready_w),
    .m_axis_tlast_o  (lb0_m_tlast_w),
    .s_axis_tdata_i  (lb0r_rdata_w[7:0]),
    .s_axis_tvalid_i (lb0r_rvalid_w),
    .s_axis_tready_o (lb0r_rready_w),
    .s_axis_tlast_i  (lb0r_rdata_w[8])
  );

  // ── CPU0: mailbox loopback FIFO (stream_id=1, simulated) ──────
  logic [7:0] lb1_m_tdata_w;
  logic       lb1_m_tvalid_w, lb1_m_tready_w, lb1_m_tlast_w;
  logic [8:0] lb1r_rdata_w;
  logic       lb1r_rvalid_w, lb1r_rready_w;

  xfcp_fifo_reg #(.DATA_WIDTH(9), .DEPTH(256)) u_lb_cpu0 (
    .clk    (clk_i), .rst_n (rst_ni), .flush (1'b0),
    .w_valid(lb1_m_tvalid_w),
    .w_data ({lb1_m_tlast_w, lb1_m_tdata_w}),
    .w_ready(lb1_m_tready_w),
    .r_valid(lb1r_rvalid_w),
    .r_data (lb1r_rdata_w),
    .r_ready(lb1r_rready_w)
  );
  ...
  xfcp_axis_adapter #(
    ...
    .clk(clk_i), .rst_n(rst_ni),
    .axis_req_valid_i   (mux_a1_req_valid_w),
    .axis_req_ready_o   (mux_a1_req_ready_w),
    .axis_req_hdr_i     (mux_a1_req_hdr_w),
    .axis_wdata_i       (mux_a1_wdata_w),
    .axis_wdata_valid_i (mux_a1_wdata_valid_w),
    .axis_wdata_ready_o (mux_a1_wdata_ready_w),
    .axis_rdata_o       (mux_a1_rdata_w),
    .axis_rdata_valid_o (mux_a1_rdata_valid_w),
    .axis_rdata_ready_i (mux_a1_rdata_ready_w),
    .axis_resp_done_o   (mux_a1_resp_done_w),
    .axis_resp_status_o (mux_a1_resp_status_w),
    .m_axis_tdata_o  (lb1_m_tdata_w),
    .m_axis_tvalid_o (lb1_m_tvalid_w),
    .m_axis_tready_i (lb1_m_tready_w),
    .m_axis_tlast_o  (lb1_m_tlast_w),
    .s_axis_tdata_i  (lb1r_rdata_w[7:0]),
    .s_axis_tvalid_i (lb1r_rvalid_w),
    .s_axis_tready_o (lb1r_rready_w),
    .s_axis_tlast_i  (lb1r_rdata_w[8])
  );
  ...
  xfcp_target_info_adapter #(
    ...
    .TARGET_TYPE ('{
      ...
      8'h01,  // 5: AXIL SEG7
      8'h01,  // 6: AXIL DIAG
      8'h02,  // 7: STREAM STR0 (sid=0)
      8'h03,  // 8: MEM MEM0
      8'h02   // 9: STREAM CPU0 (sid=1)
    }),
    .TARGET_BASE_ADDR ('{
      32'hFF00_0000,  // SYSC
      32'hFF01_0000,  // UART
      ...
      32'h0000_0001   // CPU0 stream_id=1 (low byte = sid)
    }),
    .TARGET_MAX_XFER ('{
      16'd128, 16'd128, 16'd128, 16'd128,
      16'd128, 16'd128, 16'd128,
      16'd256, 16'd256, 16'd256
    }),
    .TARGET_ALIGN ('{
      8'd4, 8'd4, 8'd4, 8'd4, 8'd4, 8'd4, 8'd4, 8'd4, 8'd4, 8'd4
    }),
    .TARGET_NAME ('{
      32'h53595343,  // "SYSC"
      32'h55415254,  // "UART"
...
endmodule : xfcp_test_11_cpu_mailbox_top

`default_nettype wire

`endif // XFCP_TEST_11_CPU_MAILBOX_TOP_SV
