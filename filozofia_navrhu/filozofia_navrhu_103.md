## Commit 39 — `socfw explain-schema`

Cieľ:

* mať rýchlu CLI referenciu bez otvárania dokumentácie
* ukázať canonical tvar pre `project`, `timing`, `ip`, `board`
* znížiť chyby typu `timing.config` vs `timing.file`
...
tests/unit/test_schema_docs.py
```
...
```python
p_schema = sub.add_parser("explain-schema", help="Show canonical YAML schema examples")
p_schema.add_argument("schema", help="Schema name: project, timing, ip, board, or list")
p_schema.set_defaults(func=cmd_explain_schema)
```
...
Commit 39 je hotový, keď:

* `socfw explain-schema project` funguje
* `socfw explain-schema timing` ukazuje správny canonical tvar
* testy pre schema docs sú green
